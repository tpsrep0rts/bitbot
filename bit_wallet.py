...
class BitWallet:
  ...
  def sell_bitcoin_by_target_pct(self, profit_pct, current_bitcoin_price):
    profit = 0.0
    qty = 0.0
    purchase_price = 0.0
    profit_multiplier = 1 + profit_pct
    for index, record in self.bitcoin_records.items():
      if record.price * profit_multiplier <= current_bitcoin_price:
